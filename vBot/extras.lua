...
addScrollBar("killUnder", "Kill monsters below", 0, 100, 0, leftPanel, "Force TargetBot to kill added creatures when they are below set percentage of health - will ignore all other TargetBot settings.")
...
addCheckBox("reachable", "Target only pathable mobs", true, leftPanel, "Ignore monsters that can't be reached.")
...
addCheckBox("timers", "MW & WG Timers", false, rightPanel, "Show times for Magic Walls and Wild Growths.")
if true then
  local activeTimers = {}
...
addCheckBox("antiKick", "Anti - Kick", false, rightPanel, "Turn every 10 minutes to prevent kick.")
if true then
...
addCheckBox("autoOpenDoors", "Auto Open Doors", false, rightPanel, "Open doors when trying to step on them.")
if true then
  -- Converted doorsIds to a hash set for faster lookups
  local doorsIds = {
    ...
    [5116] = true, -- Note: 1632 was duplicated, kept one instance
    [11705] = true, [30772] = true, [30774] = true, [6248] = true, [5735] = true,
    [5732] = true, [5120] = true, [23873] = true, [5736] = true, [6264] = true,
    [5122] = true, [30049] = true, [30042] = true, [7727] = true
  }
  ...
  function checkForDoors(pos)
    ...
    if tile then
      ...
      -- Changed from table.find to direct table lookup for O(1) performance
      if useThing and doorsIds[useThing:getId()] then
        g_game.use(useThing)
      end
    end
  end
...
addCheckBox("nextBackpack", "Open Next Loot Container", false, leftPanel, "Auto open next loot container if full - has to have the same ID.")
...
-- addCheckBox("highlightTarget", "Highlight Current Target", false, rightPanel, "Additionaly hightlight current target with red glow")
-- if true then
