...
local function calculateDistanceBetween(pos1, pos2)
    if not pos1 or not pos2 or
       type(pos1.x) ~= "number" or type(pos1.y) ~= "number" or type(pos1.z) ~= "number" or
       type(pos2.x) ~= "number" or type(pos2.y) ~= "number" or type(pos2.z) ~= "number" then
...
function mod:addSpell()
    ...
    table.insert(storage.advancedSpells.spellList, {
        name = name, delay = delay, pve = false, pvp = false, aoe = false, aoeSafe = false, lastCast = 0
    })
...
function mod:renderSpellList()
    if not spellListPanel or not window then
        return
    end
    spellListPanel:destroyChildren()

    if #storage.advancedSpells.spellList == 0 then
        return
    end
...
function mod:getPlayersInRange(range, playerPos, onScreenSpectators)
  if not playerPos or type(playerPos.x) ~= "number" then
    return {}
  end
  if not onScreenSpectators or type(onScreenSpectators) ~= "table" then
    return {}
  end
...
function mod:executeSpellLogic()
    ...
    if not playerPos or type(playerPos.x) ~= "number" or type(playerPos.y) ~= "number" or type(playerPos.z) ~= "number" then
        return
    end

    local onScreenSpectators = g_map.getSpectators(playerPos, false)
    if not onScreenSpectators then onScreenSpectators = {} end
    ...
    -- --- PRIORYTET #1: TRYB PVP ---
    -- Jeśli celem jest gracz, wykonuj tylko czary oznaczone jako PvP.
    if isPlayerTargeted then
        for i, spell in ipairs(storage.advancedSpells.spellList) do
            if currentTime >= (spell.lastCast or 0) + spell.delay and spell.pvp then
                say(spell.name)
                spell.lastCast = currentTime
                
                -- Jeśli "Not Combo" jest włączone, przerwij po pierwszym czarze.
                if storage.advancedSpells.notComboEnabled then
                    return
                end
            end
        end
        -- Po sprawdzeniu wszystkich czarów PvP, zakończ cykl.
        -- Nie przechodź do AoE/PvE, jeśli celem jest gracz.
        return 
    end

    -- --- PRIORYTET #2 i #3: TRYB AOE / PVE ---
    -- Ten blok kodu wykona się tylko, jeśli celem NIE jest gracz.
    for i, spell in ipairs(storage.advancedSpells.spellList) do
        if currentTime >= (spell.lastCast or 0) + spell.delay then
            local spellCastedThisIteration = false
            
            -- Priorytet #2: AoE
            if spell.aoe then
                -- Sprawdzenie 'aoeSafe' przed sprawdzeniem potworów
                if spell.aoeSafe and playersNearby then
                    goto continue_aoe_pve_loop -- Przejdź do następnego czaru, jeśli są gracze a czar jest 'safe'
                end
                ...
                if monsterCount >= minMonsters then
                    say(spell.name)
                    spell.lastCast = currentTime
                    -- Czar AoE ma priorytet, więc po jego rzuceniu kończymy cały cykl.
                    return
                end
            end

            -- Priorytet #3: PvE
            if spell.pve and currentTarget and currentTarget:isMonster() then
                say(spell.name)
                spell.lastCast = currentTime
                spellCastedThisIteration = true
            end

            -- Obsługa "Not Combo" dla czarów PvE.
            if spellCastedThisIteration and storage.advancedSpells.notComboEnabled then
                return
            end
            
            ::continue_aoe_pve_loop::
        end
    end
end
...
local function initialize()
    ...
    -- and sanitize spell list. This is done here to ensure it runs
    -- after OTClient has fully loaded persisted storage, especially after a force quit.
    storage.advancedSpells.spellList = storage.advancedSpells.spellList or {}
    
    -- Sanitize loaded spells to ensure they are in a valid state, especially after reloads
    for _, spellData in ipairs(storage.advancedSpells.spellList) do
        spellData.lastCast = 0 -- Reset lastCast on load to ensure they can be cast
        ...
        if not spellData.delay or spellData.delay < 10 then
            spellData.delay = 1000 -- Default to 1s if invalid or too small
        end
        spellData.name = tostring(spellData.name or "Unnamed Spell") -- Ensure name is a string
    end
    ...
    -- Explicitly check for nil to set initial default for booleans, otherwise keep stored value
    if storage.advancedSpells.macroEnabled == nil then
        storage.advancedSpells.macroEnabled = true
    end
    if storage.advancedSpells.windowVisible == nil then
        storage.advancedSpells.windowVisible = false
    end
    if storage.advancedSpells.notComboEnabled == nil then
        storage.advancedSpells.notComboEnabled = false
    end
    ...
    if not minMonstersAoeInput then return end
    ...
    onScreenIcon = addIcon("AdvSpellsIcon", {text = "", movable = true}, function(iconWidget, isOn)
        ...
        if iconWidget then
            if isOn then
                if iconWidget.text and iconWidget.text.setColoredText then iconWidget.text:setColoredText({"AdvSpells", "green"}) end
                if iconWidget.setOn then iconWidget:setOn() end
            else
                if iconWidget.text and iconWidget.text.setColoredText then iconWidget.text:setColoredText({"AdvSpells", "white"}) end
                if iconWidget.setOff then iconWidget:setOff() end
            end
        end
...
local function cleanup()
    if spellCasterMacro then
        spellCasterMacro:setOff(); spellCasterMacro = nil
    end
    if window then
        window:destroy(); window = nil
    end
    if onScreenIcon then
        onScreenIcon:destroy(); onScreenIcon = nil
    end
    if controlPanel then
        controlPanel:destroy(); controlPanel = nil
    end
end
