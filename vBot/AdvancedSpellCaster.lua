...
function mod:executeSpellLogic()
    ...
    if currentTime < sharedCooldownUntil then
        return -- Jeśli globalny cooldown po czarze AoE jest aktywny, przerwij
    end
    ...
    if currentTarget and currentTarget:isPlayer() then
        for _, spell in ipairs(storage.advancedSpells.spellList) do
            if spell.pvp and currentTime >= (spell.lastCast or 0) + spell.delay then
                say(spell.name)
                spell.lastCast = currentTime
                -- Opcja "Not Combo" działa również dla PvP
    ...
    if monsterCount >= minMonsters then
        for _, spell in ipairs(storage.advancedSpells.spellList) do
            if spell.aoe and currentTime >= (spell.lastCast or 0) + spell.delay then
                local isSafeToCast = not (spell.aoeSafe and playersNearby)
                
                if isSafeToCast then
                    say(spell.name)
                    spell.lastCast = currentTime
                    -- Ustaw globalny cooldown równy cooldownowi tego czaru AoE.
                    -- To zablokuje użycie jakichkolwiek innych czarów (w tym PvE) do czasu jego upłynięcia.
                    sharedCooldownUntil = currentTime + spell.delay
                    return -- Rzucono czar AoE, kończymy cykl
                end
            end
        end
    end

    -- --- PRIORYTET #3: TRYB PVE (Single Target) ---
    -- Wykona się tylko, jeśli nie ma warunków na AoE i nie ma globalnego cooldownu
    if currentTarget and currentTarget:isMonster() then
        for _, spell in ipairs(storage.advancedSpells.spellList) do
            if spell.pve and currentTime >= (spell.lastCast or 0) + spell.delay then
                say(spell.name)
                spell.lastCast = currentTime
                -- Jeśli "Not Combo" jest włączone, zakończ po jednym czarze PvE.
                -- Jeśli jest wyłączone, pętla będzie kontynuować i szukać kolejnych czarów PvE do combo.
                if storage.advancedSpells.notComboEnabled then
                    return
                end
            end
